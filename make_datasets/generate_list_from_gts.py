...
def main():
    ...
    parser.add_argument('--gts_subdir', default='', help='GT subdir under datasets/Subtitles')
    ...
    subtitles_root = os.path.join(project_root, 'datasets', 'car_the_garden_txt')
    print(subtitles_root)
    gts_dir = os.path.join(subtitles_root, args.gts_subdir)

    if args.output_list is None:
        out_list = os.path.join(subtitles_root, 'car_the_garden_list.txt')
    else:
